...
def neural_net(layer_sizes):
    model = tf.keras.Sequential() # 加上了 tf.keras. 前缀
    model.add(tf.keras.layers.InputLayer(input_shape=(layer_sizes[0],))) # 加上了 tf.keras.
    for width in layer_sizes[1:-1]:
        model.add(tf.keras.layers.Dense( # 加上了 tf.keras.
...
def fit(x_f, t_f, x0, t0, u0, x_lb, t_lb, x_ub, t_ub, col_weights, u_weights, tf_iter, newton_iter):
    ...
    loss_history = [] # <--- 新增：用于记录 Adam 阶段的 Loss
    ...
    for epoch in range(tf_iter):
        ...
        loss_history.append(loss_value.numpy()) # <--- 新增：每个 epoch 结束后记录当前总 Loss
    ...
    return loss_history # <--- 新增：在 fit 函数最后返回记录好的数据
...
# 改为下面这样：
loss_history = fit(x_f, t_f, x0, t0, u0, x_lb, t_lb, x_ub, t_ub, col_weights, u_weights, tf_iter = 10000, newton_iter = 10000)
...
# 顺手修好了 extent，改成 Allen-Cahn 正确的 t=[0,1], x=[-1,1]
ec = plt.imshow(FU_pred.T, interpolation='nearest', cmap='rainbow',
            extent=[0.0, 1.0, -1.0, 1.0], 
            origin='lower', aspect='auto')

ax.set_xlabel('$t$') # 注意横轴是时间 t
ax.set_ylabel('$x$') # 纵轴是空间 x
cbar = plt.colorbar(ec)
cbar.set_label('PDE residual prediction')
plt.show()
...
# # 1. 计算绝对误差 (SA-PINN 中的变量名是 U_pred 和 Exact_u)
# Error_SA = np.abs(U_pred.T - Exact_u)

# # 2. 生成完全一致的网格
# # 注意：你的 SA-PINN 代码中已经有了 t 和 x 这两个一维数组
# XX1, XX2 = np.meshgrid(t, x)

# # 3. 开启新画布并使用和 AMAW 完全一样的 pcolor 渲染方式
# plt.figure()
# # 使用同样的 shading='auto' 和 cmap='jet'
# plt.pcolor(XX1, XX2, Error_SA, shading='auto', cmap='jet')
# plt.colorbar()

# # 4. 完全复刻 AMAW 的字体大小和标签
# plt.xlabel('$t$', fontsize=20)
# plt.ylabel('$x$', fontsize=20)
# plt.title('$Error$', fontsize=20)
# plt.tight_layout()

# # 5. 保存并显示 (文件名区分开，叫 sapinn_error.pdf)
# plt.savefig('sapinn_error.pdf')
# plt.show()
...
# 1. 创建和 AMAW 一模一样的网格
# 注意：在 SA-PINN 中，t 和 x 是一维数组
# TT, XX = np.meshgrid(t, x)

# # 2. 确保预测解的维度与网格匹配
# # SA-PINN 的 U_pred 已经是二维矩阵，根据你之前的代码，需要转置或直接 reshape
# u_pred_2d = np.reshape(U_pred.T, (TT.shape[0], TT.shape[1]))

# # 3. 开启新画布并使用完全一致的 pcolor 渲染方式
# plt.figure()
# # 使用和 AMAW 相同的 shading='auto' 和 cmap='jet'
# plt.pcolor(TT, XX, u_pred_2d, shading='auto', cmap='jet')
# plt.colorbar()

# # 4. 完全复刻 AMAW 的字体大小和标签
# plt.xlabel('$t$', fontsize=20)
# plt.ylabel('$x$', fontsize=20)
# plt.title(r'SA-PINN $u(t,x)$', fontsize=20) # 标题稍微改一下以示区分
# plt.tight_layout()

# # 5. 保存并显示
# plt.savefig('sapinn_pred.pdf')
# plt.show()

######################################################################
################### SA-PINN Time Slices (Matched to AMAW) ############
######################################################################
...
# # ---------------- 第一张图: t = 0.5 (对应索引 100) ----------------
# ax = plt.subplot(gs1[0, 0])
# # SA-PINN 这里的真实解是 Exact_u[:, 100], 预测解是 U_pred[100, :]
# ax.plot(x, Exact_u[:, 100], 'b-', linewidth=2, label='Exact')
# ax.plot(x, U_pred[100, :], 'r--', linewidth=2, label='Prediction')
# ax.set_xlabel('$x$', fontsize=20)
# ax.set_ylabel('$u$', fontsize=20) # 只有第一张图有 ylabel
# ax.set_title('$t = %.2f$' % (t[100]), fontsize=20)
# ax.axis('square')
# ax.set_xlim([-1.1, 1.1])
# ax.set_ylim([-1.1, 1.1])

# # ---------------- 第二张图: t = 0.75 (对应索引 150) ----------------
# ax = plt.subplot(gs1[0, 1])
# ax.plot(x, Exact_u[:, 150], 'b-', linewidth=2, label='Exact')
# ax.plot(x, U_pred[150, :], 'r--', linewidth=2, label='Prediction')
# ax.set_xlabel('$x$', fontsize=20)
# # ax.set_ylabel('$u$', fontsize=20) # 注释掉，对齐 AMAW
# ax.axis('square')
# ax.set_xlim([-1.1, 1.1])
# ax.set_ylim([-1.1, 1.1])
...
# # ---------------- 第三张图: t = 1 (对应索引 200) ----------------
# ax = plt.subplot(gs1[0, 2])
# ax.plot(x, Exact_u[:, 200], 'b-', linewidth=2, label='Exact')
...
# # plt.tight_layout() # 保持注释，对齐 AMAW

# plt.savefig('sapinn_slices.pdf', bbox_inches='tight')
# plt.show()

######################################################################
###################### SA-PINN Total Loss Plot #######################
######################################################################

plt.figure(figsize=(8, 6))
# 画出 Adam 阶段的 10000 轮 loss 下降曲线
plt.plot(loss_history, 'b-', linewidth=2, label='Total Loss (SA-PINN)')

# 极其重要：Loss 曲线一定要用对数坐标轴 (Log scale)，才能看出量级下降！
plt.yscale('log') 

plt.xlabel('Epoch', fontsize=20)
plt.ylabel('Loss Value', fontsize=20)
plt.title('SA-PINN Training Loss', fontsize=20)
plt.grid(True, which="both", ls="--", alpha=0.5) # 加上网格线显得更专业
plt.legend(fontsize=16, frameon=False)
plt.tight_layout()

# 保存并显示图片
plt.savefig('sapinn_loss_curve.pdf', bbox_inches='tight')
plt.show()
