...
class Model:
    def __init__(self, net, x_bc_left, x_bc_right, #***
                 x_ic,
                 u_ic, x_f_loss_fun,
                 x_test, x_test_exact
                 ):
        ...
        self.x_bc_left = x_bc_left    #***
        self.x_bc_right = x_bc_right  #***
    ...
    def epoch_loss(self):
        ...
        x_left = Variable(self.x_bc_left, requires_grad=True)
        u_left = self.train_U(x_left)
        d_left = torch.autograd.grad(u_left, x_left, grad_outputs=torch.ones_like(u_left), create_graph=True)[0]
        u_x_left = d_left[:, 1].unsqueeze(-1)  

        x_right = Variable(self.x_bc_right, requires_grad=True)
        u_right = self.train_U(x_right)
        d_right = torch.autograd.grad(u_right, x_right, grad_outputs=torch.ones_like(u_right), create_graph=True)[0]
        u_x_right = d_right[:, 1].unsqueeze(-1)
    ...
    def train(self):
        ...
        self.x_ic_s = is_cuda(-torch.log(torch.tensor(100.).float()))
...
def draw_residual():
    ...
    plt.tight_layout()
    plt.savefig('AC_residual-PINN.pdf')
    plt.show()
...
def draw_epoch_w():
    ...
    plt.legend()
    plt.savefig('AC_S_RAM-AW.pdf', fontsize=20)
    plt.show()
...
if __name__ == '__main__':
    ...
    net = is_cuda(Net_attention(layers)) #use attention net
    ...
    adam_iter, lbgfs_iter = 5000, 20000
    ...
    t_bc = torch.rand([Nbc, 1])
    x_boundary_left = torch.cat((t_bc, torch.full([Nbc, 1], -1.0)), dim=1)
    x_boundary_right = torch.cat((t_bc, torch.full([Nbc, 1], 1.0)), dim=1)

    x_initial_label = (x_initial[:, [1]] ** 2) * torch.cos(np.pi * x_initial[:, [1]])

    x_ic = is_cuda(x_initial)
    u_ic = is_cuda(x_initial_label)
    
    x_bc_left = is_cuda(x_boundary_left)
    x_bc_right = is_cuda(x_boundary_right)
